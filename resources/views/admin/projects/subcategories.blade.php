...
<div class="page-header">
	...
	<div class="d-flex">
		<div class="justify-content-center"><button class="btn btn-primary my-2 btn-icon-text add_subcategory" data-target="#add_subcategory" data-toggle="modal"><i class="fe fe-plus mr-2"></i>Add Subcategory</button>
		</div>
	</div>
</div>
<!-- End Page Header -->
...
@section('scripts')
    <script>
        var defaultUrl = "{{ route('projects.getSubcategories') }}";
        var table;
                ...
                columns: [
                    ...
                    {data: 'name'},
                    {
                        ...
                        render: function (data, type, row) {
                            return "<button class='btn btn-primary btn-sm btn-flat js-edit' data-url='/Administration/project/subcategories/show/" + row.id + "' data-id='" + row.id + "'> " +
                                "<i class='fa fa-edit'></i>Edit</button>" +
                                "<button class='btn btn-danger btn-sm btn-flat js-delete ' data-id='" + data +
                                "' data-url='/Administration/project/subcategories/delete/" + row.id + "'> <i class='fa fa-trash'></i>Delete</button>";
                        }
                    }
                ]
            });
                ...
                $.ajax({
                    ...
                    if (data.subcategory=="ok"){
                        ...
                        $('#add-messages').html('<div class="alert alert-success flat">' +
                            ...
                            '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> Project Subcategory successfully saved. </div>');
            ...
            manageTable.on('click', '.js-edit', function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $("#edit-name").val(response.subcategory.name);
                        $("#edit-category").val(response.subcategory.category_id);

                        // add hidden id
                        $('#id').val(response.subcategory.id);
                        // update - form
...
//                                form[0].reset();
                                ...
                                $('#edit-messages').html('<div class="alert alert-success">' +
                                    '<button type="button" class="close" data-dismiss="alert">&times;</button>' +
                                    '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> Project Subcategory successfully updated. </div>');
