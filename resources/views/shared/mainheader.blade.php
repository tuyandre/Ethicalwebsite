			...
			<div class="main-header side-header sticky">
				<div class="container-fluid">
					...
					<div class="main-header-right">
						...
						<?php
                    $members=\App\Models\User::where('confirmed',false)->get();
                    $member2s=\App\Models\User::where('confirmed',false)->limit(3)->get();
                    ?>
							<a class="nav-link icon" href="#">
								<i class="fe fe-bell header-icons"></i>
								<span class="badge badge-danger nav-link-badge">{{$members->count()}}</span>
							</a>
							<div class="dropdown-menu">
								<div class="header-navheading">
									<p class="main-notification-text">You have {{$members->count()}} New Members<span class="badge badge-pill badge-primary ml-3">View all</span></p>
								</div>
								<div class="main-notification-list">
									<div class="media new">
										<div class="main-img-user online"><img alt="avatar" src="assets/img/users/5.jpg"></div>
										<div class="media-body">
											@foreach($member2s as $member)
										<?php
                                                $date=\Carbon\Carbon::now();
                                                ?>
											<p>A new member named <strong>{{$member->first_name}} {{$member->last_name}}</strong> with email {{$member->email}} joined the panel</p><span>{{$date->diffForHumans($member->created_at)}}</span>
										@endforeach</div>
									</div>
									<div class="media">
                        ...
                        ?>
                        @if(empty($photo))
								<img alt="avatar" src="{{ asset('dashboard/img/user.png')}}"></span>
							@else
							<img alt="avatar" src="{{ asset('uploads/profiles/'.$photo)}}"></span>
							 @endif
							</a>
							<div class="dropdown-menu">
								<div class="header-navheading">
								...
								<a class="dropdown-item" href="{{ route('profiles.getInfo') }}">
									<i class="fe fe-edit"></i> Edit Account
								</a>
								<a class="dropdown-item" href="{{ route('profiles.getPassword') }}">
									<i class="fe fe-settings"></i> Change Password
								</a>
								<a class="dropdown-item" href="{{ route('profiles.getProfile') }}">
									<i class="fe fe-settings"></i> Change Profile
								</a>
